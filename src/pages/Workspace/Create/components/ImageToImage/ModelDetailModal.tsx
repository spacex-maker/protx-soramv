...
import { normalizeUrl, isImageUrl } from './utils';
import {
  likeModel,
  unlikeModel,
  favoriteModel,
...
const ModelDetailModal: React.FC<ModelDetailModalProps> = ({ open, onClose, model }) => {
  ...
  const handleDownload = () => {
    ...
    if (coverImage) {
      ...
      const ext = coverImage?.match(/\.(jpg|jpeg|png|gif|webp)$/i)?.[0]?.slice(1) || 'jpg';
      link.download = `${model?.modelName}-cover.${ext}`;
    ...
    <ModalOverlay open={open} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <Container>
        ...
        <ContentPanel>
          ...
          <FeatureList>
            <FeatureTag>
              <CheckOutlined /> {intl.formatMessage({ id: 'create.model.capability.t2i', defaultMessage: 'T2I (Text to Image)' })}
            </FeatureTag>
            <FeatureTag>
              <CheckOutlined /> {intl.formatMessage({ id: 'create.model.capability.i2i', defaultMessage: 'I2I (Image to Image)' })}
            </FeatureTag>
          </FeatureList>
          ...
          <InfoGrid>
            ...
            {(model.imageDefaultResolution || model.imageMaxResolution) && (
              <InfoCard>
                ...
                <div className="card-content">
                  {model.imageDefaultResolution && <div>{intl.formatMessage({ id: 'create.model.resolution.default', defaultMessage: 'Default' })}: {model.imageDefaultResolution}</div>}
                  {model.imageMaxResolution && <div style={{ fontSize: 12, opacity: 0.7 }}>{intl.formatMessage({ id: 'create.model.resolution.max', defaultMessage: 'Max' })}: {model.imageMaxResolution}</div>}
                </div>
              </InfoCard>
            )}
            ...
            {model.imageFormats && (
              <InfoCard>
                ...
                <div className="card-content" style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {model.imageFormats.split(',').slice(0, 3).map((format) => (
                    <Tag key={format} style={{ margin: 0, fontSize: 10 }}>{format.toUpperCase()}</Tag>
                  ))}
                  {model.imageFormats.split(',').length > 3 && (
                    <Tag style={{ margin: 0, fontSize: 10 }}>+{model.imageFormats.split(',').length - 3}</Tag>
                  )}
                </div>
              </InfoCard>
            )}
